...
/// Responsibility: 埋め込みチャンクの保持・検索（類似度計算、RAG文脈生成）を提供。
/// Depends on: Chunk, EmbeddingModel, ModelManager (for ragAnswer), Foundation
...
class VectorStore {
    ...
    func findRelevant(queryEmbedding: [Float], topK: Int = 3) -> [Chunk] {
        let _log = SystemLog()
        let _t0 = Date()
        _log.logEvent(event: "[VectorStore] findRelevant enter dim=\(queryEmbedding.count) topK=\(topK) corpus=\(chunks.count)")
        defer {
            let dt = Date().timeIntervalSince(_t0)
            _log.logEvent(event: String(format: "[VectorStore] findRelevant exit (%.2f ms)", dt*1000))
        }
        guard !chunks.isEmpty else { return [] }
    ...
    public func ragAnswer(for query: String, topK: Int = 3) -> String {
        let _log = SystemLog()
        let _t0 = Date()
        _log.logEvent(event: "[VectorStore] ragAnswer enter qLen=\(query.count) topK=\(topK)")
        defer {
            let dt = Date().timeIntervalSince(_t0)
